...
if (!(M = Number(window.location.hash.slice(1)))) {
    M = 1000
}
...
async function setup(){
    let svg = await fetch("./public/kuri.svg")
        .then(response => response.text())
        .then(text => (new DOMParser).parseFromString(text, "image/svg+xml"))
        .then(svg => svg.documentElement);

    createCanvas(1080, 720);
    zoom = createSlider(5,M*10,5,0);
    speed = createSlider(1,10,1,0.5);
    circles = createSlider(10,2000,M,10);
    reloadButton = createButton('Regenerate')
...
function reload() {
    window.location.href = `#${circles.value()}`
    location.reload()
}